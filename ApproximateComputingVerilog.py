...
def FPGA_Based_VerilogAdder():                      # FPGA Based Verilog Adder (Generalized Adder)
    ...
    while(True):
        ...
        if(adder_type_inp=='q' or adder_type_inp=='Q'):
            ...
            exit_flag=0;
            break;
        elif(adder_type_inp.isnumeric() and int(adder_type_inp)>=1 and int(adder_type_inp)<=total_num_adder_types):
            ...
            if(adder_type_inp=='1'):
                ...
                if(num_inacc_bits>num_bits-1 or num_inacc_bits<3):
                    print("\nWARNING - The number of inaccurate bits should lie between 3 and "+ str(num_bits-1) +"! Please try again!\n");
                    continue;
                if(adder_type_inp=='2'):
    ...
    #    num_inacc_bits=8;
        ...
        if(num_bits>1):
            a_vec=vast.Ioport(vast.Input(inp_a,vast.Width(vast.IntConst(ascii(num_bits-1)),vast.IntConst('0'))));
            b_vec=vast.Ioport(vast.Input(inp_b,vast.Width(vast.IntConst(ascii(num_bits-1)),vast.IntConst('0'))));
        elif(num_bits==1):
            a_vec=vast.Ioport(vast.Input(inp_a));
            b_vec=vast.Ioport(vast.Input(inp_b));
            
        clk=vast.Ioport(vast.Input(input_clk));
        sum_vec=vast.Ioport(vast.Output(output_sum,vast.Width(vast.IntConst(ascii(num_bits)),vast.IntConst('0'))),vast.Reg(output_sum,vast.Width(vast.IntConst(ascii(num_bits)),vast.IntConst('0'))));
        ...
        if(num_bits>1):
            reginp1=vast.Reg(input_a,vast.Width(vast.IntConst(ascii(num_bits-1)),vast.IntConst('0')));
            reginp2=vast.Reg(input_b,vast.Width(vast.IntConst(ascii(num_bits-1)),vast.IntConst('0')));
        elif(num_bits==1):
        ...
        item1=[vast.Always(vast.Sens(vast.Identifier(input_clk), type='posedge'),vast.Block(inp_2_reg))];


        if(AdderType=='Accurate_Adder'):
            item2=AdderSumCalc.accurate_sum_calc(num_bits,input_a,input_b,input_clk,output_sum);
            file=AdderType+'_'+NumAdderBits+'_bits';
        elif(AdderType=='HEAA_Adder'):
            item2=AdderSumCalc.HEAA_sum_calc(num_bits,num_inacc_bits,input_a,input_b,input_clk,output_sum);
        elif(AdderType=='HOERAA_Adder'):
            item2=AdderSumCalc.HOERAA_sum_calc(num_bits,num_inacc_bits,input_a,input_b,input_clk,output_sum);
        elif(AdderType=='HOAANED_Adder'):
            item2=AdderSumCalc.HOAANED_sum_calc(num_bits,num_inacc_bits,input_a,input_b,input_clk,output_sum);
        elif(AdderType=='HERLOA_Adder'):
            item2=AdderSumCalc.HERLOA_sum_calc(num_bits,num_inacc_bits,input_a,input_b,input_clk,output_sum);
        elif(AdderType=='M_HERLOA_Adder'):
            item2=AdderSumCalc.M_HERLOA_sum_calc(num_bits,num_inacc_bits,input_a,input_b,input_clk,output_sum);

        if(AdderType=='Accurate_Adder'):
            file=AdderType+'_'+NumAdderBits+'_bits';
        else:
            file=AdderType+'_'+NumAdderBits+'_bits_'+inaccurate_bits_str+'_inacc';
...
def ASIC_Based_VerilogAdder():                  # ASIC Based Verilog Adder (Generalized Adder)
    ...
    while(True):
        ...
        if(adder_type_inp=='q' or adder_type_inp=='Q'):
            ...
            if(adder_type_inp=='1'):
                verilog_code = VerilogStructuralAdder.HEAA_Generic(num_bits,num_inacc_bits).to_verilog();
                ...
                verilog_code = VerilogStructuralAdder.HOERAA_Generic(num_bits,num_inacc_bits).to_verilog();
...
##                file='hoeraa_32b.v';
                ...
                verilog_code = VerilogStructuralAdder.HOAANED_Generic(num_bits,num_inacc_bits).to_verilog();
                file='hoaaned_'+str(num_bits)+'b'+str(num_inacc_bits)+'inacc.v';
##                verilog_code = VerilogStructuralAdder.HOAANED_32_bits_10_inaccurate().to_verilog();
##                file='hoaaned_32b.v';
            elif(adder_type_inp=='4'):
                verilog_code = VerilogStructuralAdder.M_HERLOA_Generic(num_bits,num_inacc_bits).to_verilog();
                file='m_herloa_'+str(num_bits)+'b'+str(num_inacc_bits)+'inacc.v';
...
##        print('20. PAAM-01 v7 8x8 Approximate Multiplier');
        ...
        if(multiplier_type_inp=='q' or multiplier_type_inp=='Q'):
            ...
            if(multiplier_type_inp.isnumeric() and int(multiplier_type_inp)>=1 and int(multiplier_type_inp)<=total_num_multiplier_types):
                ...
                if(multiplier_type_inp=='1'):
                    verilog_code = VerilogMultiplierCode.accurate_MxN_multiplier(M,N).to_verilog();
                    file='multiplier_'+str(M)+'x'+str(N)+'.v';
                elif(multiplier_type_inp=='2'):
                    verilog_code = VerilogMultiplierCode.accurate_MxN_binary_array_multiplier(M,N).to_verilog();
                    file='bam_'+str(M)+'x'+str(N)+'.v';
                elif(multiplier_type_inp=='3'):
                    V_cut_str = input('Enter position of V-cut (between 0 and ' + str(M)+ ' both inclusive) - \t');
                    V_cut=int(V_cut_str);
                    if(V_cut>M or V_cut<0):
                        print("\nWARNING - The position of V-cut should lie between 0 and "+ str(M) +" (both inclusive)! Please try again!\n");
                        continue;
                    verilog_code = VerilogMultiplierCode.PAAM01_V_cut_MxN_binary_array_multiplier(M,N,V_cut).to_verilog();
                    file='paam_01_V_'+str(V_cut)+'_'+str(M)+'x'+str(N)+'.v';
