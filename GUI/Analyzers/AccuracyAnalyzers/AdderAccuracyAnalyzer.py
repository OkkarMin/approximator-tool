...
def analyze(user_chosen_options):
    inacc_bits, total_bits, adder_first_unsigned_number, adder_second_unsigned_number, type_of_accuracy_analysis_hardware = extract_options(
        user_chosen_options)
    ...
    inaccurate_adder_sum = getattr(
        ApproxAdders, f'{type_of_accuracy_analysis_hardware}_approx')(
            adder_first_unsigned_number, adder_second_unsigned_number,
            total_bits, inacc_bits)

    inaccurate_adder_accuracy = (
        1 - (abs(accurate_adder_sum - inaccurate_adder_sum) /
             accurate_adder_sum)) * 100

    print_analysis(accurate_adder_sum, inaccurate_adder_sum,
                   inaccurate_adder_accuracy,
...
def extract_options(user_chosen_options):
    ...
    type_of_accuracy_analysis_hardware = user_chosen_options[
        'type_of_accuracy_analysis_hardware']
    return inacc_bits, total_bits, adder_first_unsigned_number, adder_second_unsigned_number, type_of_accuracy_analysis_hardware


def print_analysis(accurate_adder_sum, inaccurate_adder_sum,
                   inaccurate_adder_accuracy,
                   type_of_accuracy_analysis_hardware):
