...
def PAAM01_VCut(N1, N2, V_val):
    ...
    if N1 <= 10 and N2 <= 10:
        ...
        for num1 in range(2 ** N1):
            for num2 in range(2 ** N2):
                ...
                PAAM01_VCut_estimate_RMSE += (
                    PAAM01_VCut_product - accurate_product
                ) ** 2
        ...
        print("PAAM01 VCut=", V_val, "mean absolute error", PAAM01_VCut_estimate_MAE)
        print()

        PAAM01_VCut_estimate_RMSE /= 2 ** (N1 + N2)
        PAAM01_VCut_estimate_RMSE = math.sqrt(PAAM01_VCut_estimate_RMSE)
        print(
            "PAAM01 VCut=", V_val, "Root Mean Square Error", PAAM01_VCut_estimate_RMSE
        )
        print()
...
def AAM01_VCut(N1, N2, V_val):
    ...
    if N1 <= 10 and N2 <= 10:
        ...
        PAAM01_VCut_estimate_AE /= 2 ** (N1 + N2)
        print()
        print("AAM01 VCut=", V_val, "average error", PAAM01_VCut_estimate_AE)
        print()

        PAAM01_VCut_estimate_MAE /= 2 ** (N1 + N2)
        print("AAM01 VCut=", V_val, "mean absolute error", PAAM01_VCut_estimate_MAE)
        print()

        PAAM01_VCut_estimate_RMSE /= 2 ** (N1 + N2)
        PAAM01_VCut_estimate_RMSE = math.sqrt(PAAM01_VCut_estimate_RMSE)
        print(
            "AAM01 VCut=", V_val, "Root Mean Square Error", PAAM01_VCut_estimate_RMSE
        )
        print()
