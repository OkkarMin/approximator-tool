...
class FPGA_Based_VerilogAdder_Generator:
    ...
    def generate_verilog_code(user_chosen_options):
        ...
        if total_bits > 1:
            a_vec = vast.Ioport(
                vast.Input(
                    inp_a,
                    vast.Width(
                        vast.IntConst(ascii(total_bits - 1)), vast.IntConst("0")
                    ),
                )
            )
            b_vec = vast.Ioport(
        ...
        sum_vec = vast.Ioport(
            vast.Output(
                output_sum,
                vast.Width(vast.IntConst(ascii(total_bits)), vast.IntConst("0")),
            ),
            vast.Reg(
                output_sum,
                vast.Width(vast.IntConst(ascii(total_bits)), vast.IntConst("0")),
        ...
        if type_of_hardware_module == "Accurate Adder":
            item2 = AdderSumCalc.accurate_sum_calc(
                total_bits, input_a, input_b, input_clk, output_sum
            )
