...
def VerilogCodeGeneratorTabLogic(window, event, values):
    ...
    type_of_hardware_module = values[f"{type_of_verilog_code}_Hardware_Type"][0]
    ...
    # V-cut input should only show up for 'MxN AAM01 with V-cut'
    if type_of_hardware_module == "MxN AAM01 with V-cut":
    ...
    if event == "Generate":
        ...
        user_chosen_options["type_of_hardware_module"] = values[
            f"{type_of_verilog_code}_Hardware_Type"
        ][0]
        ...
        is_valid, error_message = validate(
            user_chosen_options, "VerilogCodeGeneratorTabValidator"
        )
