    ...
    while(True):
        ...
        if(adder_type_inp=='q' or adder_type_inp=='Q'):
            ...
            if(num1>(2**num_bits-1) or num1<0):
                print("\nWARNING - The number should lie between 0 and "+ str(2**num_bits-1) +"! Please try again!\n");
                continue;
            ...
            if(adder_type_inp=='1'):
                HEAA_sum=ApproxAdders.HEAA_approx(num1,num2,num_bits,num_inacc_bits);
                print("\nHEAA Approx Adder Sum:\t",HEAA_sum,"\n");
                HEAA_Accuracy=(1-(abs(accurate_sum-HEAA_sum)/(accurate_sum)))*100;
                if(HEAA_Accuracy<0):
                    HEAA_Accuracy=0;
                print("\nHEAA Approx Adder Accuracy:\t",HEAA_Accuracy,"\n");
                
            elif(adder_type_inp=='2'):
                HOERAA_sum=ApproxAdders.HOERAA_approx(num1,num2,num_bits,num_inacc_bits);
                print("\nHOERAA Approx Adder Sum:\t",HOERAA_sum,"\n");
                HOERAA_Accuracy=(1-(abs(accurate_sum-HOERAA_sum)/(accurate_sum)))*100;
                if(HOERAA_Accuracy<0):
                    HOERAA_Accuracy=0;                
                print("\nHOERAA Approx Adder Accuracy:\t",HOERAA_Accuracy,"\n");

            elif(adder_type_inp=='3'):
                HOAANED_sum=ApproxAdders.HOAANED_approx(num1,num2,num_bits,num_inacc_bits);
                print("\nHOAANED Approx Adder Sum:\t",HOAANED_sum,"\n");
                HOAANED_Accuracy=(1-(abs(accurate_sum-HOAANED_sum)/(accurate_sum)))*100;
                if(HOAANED_Accuracy<0):
                    HOAANED_Accuracy=0;   
                print("\nHOAANED Approx Adder Accuracy:\t",HOAANED_Accuracy,"\n");

            elif(adder_type_inp=='4'):
                M_HERLOA_sum=ApproxAdders.M_HERLOA_approx(num1,num2,num_bits,num_inacc_bits);
                print("\nM-HERLOA Approx Adder Sum:\t",M_HERLOA_sum,"\n");
                M_HERLOA_Accuracy=(1-(abs(accurate_sum-M_HERLOA_sum)/(accurate_sum)))*100;
...
def ApproxMultiplierAccuracy():
    ...
    while(True):
        ...
        if(multiplier_type_inp=='q' or multiplier_type_inp=='Q'):
        ...
        elif(multiplier_type_inp.isnumeric() and int(multiplier_type_inp)>=1 and int(multiplier_type_inp)<=total_num_multiplier_types):
            ...
            if(num1>(2**N1-1) or num1<0):
                print("\nWARNING - The number should lie between 0 and "+ str(2**N1-1) +"! Please try again!\n");
                continue;
            
            num2_str = input('Enter the second number to be added (unsigned) - \t');
            num2=int(num2_str);
            if(num2>(2**N2-1) or num2<0):
                print("\nWARNING - The number should lie between 0 and "+ str(2**N2-1) +"! Please try again!\n");
                continue;

            accurate_product=ApproxMultipliers.accurate_array_multiplier(num1,num2,N1,N2);
            ...
            if(multiplier_type_inp=='1'):
                ...
                if(V_val>(N1+N2-3) or V_val<0):
                    print("\nWARNING - The position of V-cut should lie between 0 and "+ str(N1+N2-3) +"! Please try again!\n");
                    continue;

                PAAM01_VCut_product=ApproxMultipliers.PAAM01(num1,num2,N1,N2,V_val);
                print("\nPAAM01-Vcut Approx Multiplier Product:\t",PAAM01_VCut_product,"\n");
                PAAM01_VCut_Accuracy=(1-(abs(accurate_product-PAAM01_VCut_product)/(accurate_product)))*100;
                if(PAAM01_VCut_Accuracy<0):
                    PAAM01_VCut_Accuracy=0;   
                print("\nPAAM01-Vcut Approx Multiplier Accuracy:\t",PAAM01_VCut_Accuracy,"\n");
