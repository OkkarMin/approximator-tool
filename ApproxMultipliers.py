...
def accurate_array_multiplier(num1, num2, N1, N2):
    ...
    for j in range(N2):
        for i in range(N1):
            ...
            if j == 0:
                ...
                if i != (N1 - 1):
                    (res[i][j], carry[i][j]) = adder.HalfAdder(
                        AND_Gate[i][j], res[i + 1][j - 1]
                    )
                ...
                if i != (N1 - 1):
                    (res[i][j], carry[i][j]) = adder.FullAdder(
                        AND_Gate[i][j], res[i + 1][j - 1], carry[i][j - 1]
                    )
                else:
                    res[i][j] = AND_Gate[i][j]
                    carry[i][j] = 0
    ...
    for i in range(N2, N1 + N2 - 1):
        if i == N2:
            (sum_bit[i], carry_final[i - N2]) = adder.HalfAdder(
                res[i + 1 - N2][N2 - 1], carry[i - N2][N2 - 1]
            )
        else:
            (sum_bit[i], carry_final[i - N2]) = adder.FullAdder(
                res[i + 1 - N2][N2 - 1], carry[i - N2][N2 - 1], carry_final[i - N2 - 1]
            )
