    ...
    for i in range(M):
        for j in range(N):
            if(i==0 and j==0):
                ...
                exec("Mult.Instance(AndModule,'and_%d%d',ports=[w%d%d,a[%d],b[%d]]);" %(i,j,i,j,i,j));
...
##    Mult.Instance(AndModule,'mi2',ports=[res[0],a[0],b[0]]);
    ...
    for j in range(1,N+1):
        for i in range(1,M):
            if(j==1):
                if(i==1):
                    exec("Mult.Instance(HalfAdderModule,'ha%d',ports=[w%d0,w%d1,res[1],c_1%d]);" %(i,i,i-1,i-1));
                else:
                    exec("Mult.Instance(HalfAdderModule,'ha%d',ports=[w%d0,w%d1,s_1%d,c_1%d]);" %(i,i,i-1,i-1,i-1));
            elif(j!=N):
                if(i==1):
                    exec("Mult.Instance(FullAdderModule,'fa%d%d',ports=[c_%d%d,s_%d%d,w%d%d,res[%d],c_%d%d]);" %(i,j,j-1,i-1,j-1,i,i-1,j,j,j,i-1));
                elif(i!=(M-1)):
                    exec("Mult.Instance(FullAdderModule,'fa%d%d',ports=[c_%d%d,s_%d%d,w%d%d,s_%d%d,c_%d%d]);" %(i,j,j-1,i-1,j-1,i,i-1,j,j,i-1,j,i-1));
                else:
                    exec("Mult.Instance(FullAdderModule,'fa%d%d',ports=[c_%d%d,w%d%d,w%d%d,s_%d%d,c_%d%d]);" %(i,j,j-1,i-1,i,j-1,i-1,j,j,i-1,j,i-1));
            else:
                if(i==1):
...
def PAAM01_V_cut_MxN_binary_array_multiplier(M,N,V_cut):             # Accurate MxN Binary Array Multiplier
                                                           # Remember to use constraint M>=3 and N>=3
        ...
        for i in range(1,M):
            if((i+j-1)>V_cut):
                if(j==1):
                    ...
                    if((i+j-2)==V_cut):
                        if(i==1):
                            exec("Mult.Instance(HalfAdderModule,'ha%d%d',ports=[s_%d%d,w%d%d,res[%d],c_%d%d]);" %(i,j,j-1,i,i-1,j,j,j,i-1));
                        elif(i!=(M-1)):
                            exec("Mult.Instance(HalfAdderModule,'ha%d%d',ports=[s_%d%d,w%d%d,s_%d%d,c_%d%d]);" %(i,j,j-1,i,i-1,j,j,i-1,j,i-1));
                        else:
                            exec("Mult.Instance(HalfAdderModule,'ha%d%d',ports=[w%d%d,w%d%d,s_%d%d,c_%d%d]);" %(i,j,i,j-1,i-1,j,j,i-1,j,i-1));   
                    else:
                        if(i==1):
                            exec("Mult.Instance(FullAdderModule,'fa%d%d',ports=[c_%d%d,s_%d%d,w%d%d,res[%d],c_%d%d]);" %(i,j,j-1,i-1,j-1,i,i-1,j,j,j,i-1));
                        elif(i!=(M-1)):
                    ...
                    if((i+j-2)==V_cut):
                        ...
                        V_cut_flag=0;
                        if(i!=(M-1)):
                           exec("Mult.Instance(HalfAdderModule,'hadd%d',ports=[c_%d%d,s_%d%d,res[%d],c_%d%d]);" %(i-1,j-1,i-1,j-1,i,j+i-1,j,i-1));
                        else:
                           exec("Mult.Instance(HalfAdderModule,'hadd%d',ports=[c_%d%d,w%d%d,res[%d],res[%d]]);" %(i-1,j-1,i-1,i,j-1,j+i-1,j+i));
                    else:
                        if(i==1):
                            exec("Mult.Instance(HalfAdderModule,'hadd',ports=[c_%d%d,s_%d%d,res[%d],c_%d%d]);" %(j-1,i-1,j-1,i,j+i-1,j,i-1));
                        elif(i!=(M-1)):
